...
<div class="main-content">
  <div class="topbar">
    <div class="topbar-title"><h1>Daily Planner</h1><p>Organize your schedule and study sessions</p></div>
    ...
    <div style="display:grid;grid-template-columns:auto 1fr;gap:1.5rem;align-items:start">
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:0">
          <div id="schedule-list"><div style="padding:2rem;text-align:center"><span class="spinner"></span></div></div>
        </div>
      </div>
    </div>
  </div>
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select class="form-control" id="s-type">
            <option>Class</option><option>Study</option><option>Personal</option><option>Meeting</option>
          </select>
        </div>
        <div class="form-group">
          <label>Color</label>
...
const typeColors = {Class:'#1e3a5f',Study:'#3b82f6',Personal:'#22c55e',Meeting:'#f97316'};
...
function selectDay(dateStr) {
  ...
  document.getElementById('schedule-date-label').textContent =
    'Schedules — ' + new Date(dateStr + 'T00:00:00').toLocaleDateString('en-PH',{weekday:'long',month:'long',day:'numeric'});
...
async function loadSchedules() {
  ...
  const res   = await fetch(`${API}?action=get_schedules&start=${start}&end=${end} 23:59:59`);
...
function closeModal() { document.getElementById('sched-modal').classList.remove('show'); }

async function saveSchedule() {
  const title = document.getElementById('s-title').value.trim();
  const start = document.getElementById('s-start').value;
  const end   = document.getElementById('s-end').value;
  if (!title || !start || !end) { toast('Please fill in all required fields.','error'); return; }
  ...
  const data = await res.json();
  if (data.success) { toast(data.message); closeModal(); loadSchedules(); }
...
async function deleteSchedule(id) {
  ...
  const res  = await fetch(API, {method:'POST', body:fd});
  const data = await res.json();
  if (data.success) { toast('Schedule deleted.'); loadSchedules(); }
}
