...
<div class="main-content">
  <div class="topbar">
    <div class="topbar-title"><h1>My Tasks</h1><p>Manage and track your academic tasks</p></div>
  ...
  <div class="page-content">
    ...
    <div class="card">
      <div class="card-body" style="padding:0">
        <div id="tasks-list"><div style="padding:2rem;text-align:center"><span class="spinner"></span></div></div>
      </div>
    </div>
  </div>
</div>
...
async function saveTask() {
  ...
  if (!name || !due) { toast('Task name and due date are required.','error'); return; }
  ...
  const data = await res.json();
  if (data.success) { toast(data.message); closeModal(); loadTasks(); }
...
async function deleteTask(id) {
  ...
  const res  = await fetch(API, {method:'POST', body:fd});
  const data = await res.json();
  if (data.success) { toast('Task deleted.'); loadTasks(); }
  else toast(data.message, 'error');
}
