...
<div class="main-content">
  ...
  <div class="page-content">
    ...
    <div class="stats-grid" id="stats-grid">
      <div class="stat-card"><div class="stat-icon blue"><i class="fas fa-tasks"></i></div>
        <div class="stat-info"><div class="value" id="stat-pending">–</div><div class="label">Pending Tasks</div></div></div>
      <div class="stat-card"><div class="stat-icon green"><i class="fas fa-check-circle"></i></div>
        <div class="stat-info"><div class="value" id="stat-done">–</div><div class="label">Completed</div></div></div>
      <div class="stat-card"><div class="stat-icon red"><i class="fas fa-exclamation-circle"></i></div>
        <div class="stat-info"><div class="value" id="stat-overdue">–</div><div class="label">Overdue</div></div></div>
      <div class="stat-card"><div class="stat-icon orange"><i class="fas fa-calendar-alt"></i></div>
        <div class="stat-info"><div class="value" id="stat-upcoming">–</div><div class="label">Upcoming Deadlines</div></div></div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:0">
          <div id="today-tasks"><div style="padding:1.5rem;text-align:center;"><span class="spinner"></span></div></div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:0">
          <div id="upcoming-list"><div style="padding:1.5rem;text-align:center;"><span class="spinner"></span></div></div>
        </div>
      </div>
    </div>
...
async function markDone(taskId) {
  ...
  await fetch(API_STUDENT, {method:'POST',body:fd});
...
async function quickAddTask() {
  ...
  if (!name || !due) { toast('Please fill in task name and due date.','error'); return; }
  const fd = new FormData();
  fd.append('action','add_task');
  fd.append('task_name', name);
  fd.append('due_at', due);
  fd.append('priority', prio);
  const res  = await fetch(API_STUDENT, {method:'POST',body:fd});
  const data = await res.json();
  if (data.success) {
    toast('Task added!');
